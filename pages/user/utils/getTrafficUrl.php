...
// Invalidate all
$query = $BDD->prepare('DELETE FROM exchange_done WHERE (user_id=:user_id AND type=0 AND validated=0) OR ((timestamp+24*60*1000)<NOW()) LIMIT 1;');
$query->bindParam(':user_id', intval(htmlspecialchars($_SESSION['userid'])));
$query->execute();

// Get next url
$query = $BDD->prepare('SELECT id, url FROM exchange_traffic WHERE id NOT IN (SELECT traffic_id FROM exchange_done WHERE user_id=:userid AND type=0) AND available>:minimum ORDER BY priority DESC LIMIT 1;');
...
if ($query->rowCount() == 1)
{
	...
	// Set current
	$query = $BDD->prepare('INSERT INTO exchange_done VALUES (\'\', :site_id, 0, :userid, NOW(), 0);');
