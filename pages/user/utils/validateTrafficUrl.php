...
// Get timer
$query = $BDD->prepare('SELECT timestamp FROM exchange_done WHERE user_id=:userid AND validated=0 AND type=0 LIMIT 1;');
...
if ($query->rowCount() == 1 && $now >= $expected)
{
	...
	// Get website id
	$query = $BDD->prepare('SELECT traffic_id FROM exchange_done WHERE user_id=:userid AND type=0 AND validated=0 LIMIT 1;');
	$query->bindParam(':userid', $user_id, PDO::PARAM_INT);
	$query->execute();
	$website_id = $query->fetchColumn();

	// Set the website as validated
	$query = $BDD->prepare('UPDATE exchange_done SET validated=1 WHERE user_id=:userid AND type=0 AND validated=0 LIMIT 1;');
	$query->bindParam(':userid', $user_id, PDO::PARAM_INT);
	$query->execute();
	...
	$query = $BDD->prepare('UPDATE exchange_traffic SET available=available-:points, done=done+1 WHERE id=:site_id LIMIT 1;');
